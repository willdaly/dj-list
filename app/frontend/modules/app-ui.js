...
import { initSlider, getBpmRange } from './slider';
...
function selectSongsToAdd() {
  ...
  if (fakeArray.length > 0) {
    ...
    var songIdsArray = $.map(realArray, function(row) { return row.id; });
    return songIdsArray;
  }
  noSongSelected();
  return null;
...
function addToPlaylist() {
  ...
  if (songIdsArray) {
    ...
    $.ajax({
      ...
      success: () => {
        $('#message').empty().append("<a data-targer='#'>playlist updated</a>");
        $('#message a').delay(2500).fadeOut(500, () => { $('#message a').remove(); });
      }
    });
  }
...
function transpose() {
  ...
  $.ajax({
    url: '/transpose',
    type: 'POST',
    data: { trans: trans, BPM: lowBpm, Key: keyValue, genre: genreArray() },
    success: (response) => {
      appendSearchResults(response.songs);
    }
  });
...
function bpm(e) {
  ...
  $.ajax({
    url: '/bpm',
    type: 'POST',
    data: { BPM: [range[0], range[1]], genre: array },
    success: (response) => {
      appendSearchResults(response.songs);
    }
  });
...
function bpmKey(e) {
  ...
  $.ajax({
    url: '/bpmKey',
    type: 'POST',
    data: { BPM: [range[0], range[1]], Key: keyValue, genre: array },
    success: (response) => {
      appendSearchResults(response.songs);
    }
  });
