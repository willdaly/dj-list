...
(function(){
  ...
  function editSong () {
    ...
    $.ajax({
      url: '/editSong',
      type: 'put',
      data: {Id: id, BPM: bpm, Key: key, Title: title, Artist: artist, genre: genre},
      success: response => {
        $('#editSong').modal('hide');
        $('#message').empty().append(`<a data-target='#'>${response.song.Song} updated</a>`); //change title to like response.song.title
        $('#message a').delay( 2500 ).fadeOut(500, ()=>{$('#message a').remove();} );
      }
    });
  }
  ...
  function updateOrder (title, oldOrder, newOrder) {
    ...
    $.ajax({
      url: '/updateOrder',
      type: 'PUT',
      data: {playlistId: playlistId, songTitle: title, oldOrder: oldOrder, newOrder: newOrder},
      success: response => {
        appendPlaylistSongs(response.playlist.songs);
      }
    });
